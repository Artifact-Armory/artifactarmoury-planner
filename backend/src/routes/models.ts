...
import { notifyFollowersOfRelease, notifyAdminsOfMeshOverride } from '../services/notifications';
...
router.post('/from-upload',
  ...
  asyncHandler(async (req, res) => {
    ...
    // Background processing — seller polls GET /:id. Goes to the ingest worker
    // queue when MODEL_INGEST_WORKER_ENABLED is on, otherwise runs in-process
    // exactly as before (see services/modelIngest/queue.ts).
    await dispatchIngestUpload({
      modelId: model.id,
      rawKey,
      filename,
...
router.get('/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    // stl_file_path/source_format/source_file_path are pulled only to size the
    // download below — stripped before anything reaches the client, same as the
    // model row's own stl_file_path (see the SECURITY note further down).
    let parts: any[] = [];
    ...
    // Total bytes the buyer's download will actually transfer (the STL, or a
    // ZIP of every part for a multi-part set) — shown on the product page so a
    // heavy listing doesn't just silently take minutes with no explanation.
    const downloadSizeBytes = await getEstimatedDownloadBytes(id, model, parts);
...
// ============================================================================
// The actual work (processUploadedModel / processModelVersionUpdate /
// processModelParts / findGeometryDuplicate / markModelFailed /
// safeDeleteObject) moved to services/modelIngest/process.ts on 2026-09-03, so
// it can run either in-process (as before) or in the separate worker service
// via services/modelIngest/queue.ts's dispatchIngestUpload /
// dispatchIngestVersionUpdate (see migration 057, MODEL_INGEST_WORKER_ENABLED).
// findGeometryDuplicate and safeDeleteObject are still used directly in this
// file (the pre-flight checks above, and the legacy multipart create route),
// so they're imported back in rather than duplicated.
