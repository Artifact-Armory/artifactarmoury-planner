...
// Default to relative API path so Vite proxy (dev) and same-origin (prod) just work.
...
apiClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig): InternalAxiosRequestConfig => {
    ...
    const token =
      typeof window !== 'undefined' ? localStorage.getItem('terrain_builder_token') : null
      ...
      try {
        const refreshToken =
          typeof window !== 'undefined'
            ? localStorage.getItem('terrain_builder_refresh_token')
            : null
        if (!refreshToken) {
        ...
        const refreshSessionHeader =
          ...
          response.headers?.[SESSION_HEADER.toLowerCase()]
        syncSessionFromResponse(
