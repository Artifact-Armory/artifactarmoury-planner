...
export const artistsApi = {
  ...
  getAnalytics: async (artistId: string): Promise<ArtistAnalytics> => {
    const response = await apiClient.get(`/api/artists/${artistId}/analytics`)
    const payload = response.data?.analytics ?? response.data
    return mapArtistAnalytics(payload)
  },

  updateProfile: async (artistId: string, data: {
    name?: string
    bio?: string
    profileImage?: string
    bannerImage?: string
  }): Promise<ArtistDetail> => {
    const response = await apiClient.put(`/api/artists/${artistId}/profile`, data)
    return mapArtistDetail(response.data?.artist ?? response.data)
  },
}
