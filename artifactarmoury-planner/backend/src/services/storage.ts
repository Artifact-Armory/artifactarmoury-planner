...
const STORAGE_MODE = process.env.STORAGE_MODE || 'local' // 'local', 's3', or 'hybrid'
...
export async function initializeStorage(): Promise<void> {
  try {
    logger.info(`Initializing storage (mode: ${STORAGE_MODE})...`)

    // Initialize S3 if using S3 or hybrid mode
    if (STORAGE_MODE === 's3' || STORAGE_MODE === 'hybrid') {
      try {
        s3Storage.initializeS3Client()
        logger.info('✓ S3 storage initialized')
      } catch (error) {
        logger.error('Failed to initialize S3 storage', { error })
        if (STORAGE_MODE === 's3') {
          throw error // Fail if S3 is required
        }
        // Continue with local storage if hybrid mode
      }
    }

    // Create local storage directories (always needed for temp files)
    for (const [name, dirPath] of Object.entries(STORAGE_PATHS)) {
      await fs.mkdir(dirPath, { recursive: true })
      logger.debug(`Created storage directory: ${name}`)
    }

    logger.info(`✓ Storage initialized successfully (mode: ${STORAGE_MODE})`)
  } catch (error) {
...
export async function saveFile(options: SaveFileOptions): Promise<SaveFileResult> {
  ...
  try {
    ...
    // Build S3 key path
    let s3Key: string = category
    if (artistId) {
      s3Key = `${s3Key}/${artistId}`
    }
    if (assetId) {
      s3Key = `${s3Key}/${assetId}`
    }
    ...
    if (STORAGE_MODE === 's3' || STORAGE_MODE === 'hybrid') {
      try {
        await s3Storage.uploadFile(s3Key, buffer, mimetype, {
          'original-name': originalName,
          'category': category,
          'artist-id': artistId || 'unknown',
          'asset-id': assetId || 'unknown'
        })
        logger.debug('File saved to S3', {
          filename,
          s3Key,
          size: buffer.length,
          category
        ...
        logger.error('Failed to save file to S3', { error, s3Key })
        if (STORAGE_MODE === 's3') {
          throw error // Fail if S3 is required
        }
        // Continue with local storage if hybrid mode
    ...
    // Save to local storage if not S3-only mode
    if (STORAGE_MODE === 'local' || STORAGE_MODE === 'hybrid') {
      await fs.mkdir(dirPath, { recursive: true })
    ...
    const relativePath = STORAGE_MODE === 's3' ? s3Key : path.relative(STORAGE_ROOT, path.join(dirPath, filename))
    ...
    return {
      filename,
      filepath: STORAGE_MODE === 's3' ? s3Key : path.join(dirPath, filename),
      relativePath,
      size: buffer.length,
      mimetype
    }
...
export async function readFile(filePath: string): Promise<Buffer> {
  try {
    // Check if this is an S3 key (doesn't start with /)
    const isS3Key = !filePath.startsWith('/') && !filePath.startsWith('.')

    if (isS3Key && (STORAGE_MODE === 's3' || STORAGE_MODE === 'hybrid')) {
      try {
        return await s3Storage.downloadFile(filePath)
      } catch (error) {
        logger.error('Failed to read file from S3', { error, filePath })
        if (STORAGE_MODE === 's3') {
          throw error
        }
        // Fall through to local storage if hybrid mode
      }
    }
...
export async function getFileStream(filePath: string): Promise<NodeJS.ReadableStream> {
  try {
    ...
    if (isS3Key && (STORAGE_MODE === 's3' || STORAGE_MODE === 'hybrid')) {
      try {
        return await s3Storage.getFileStream(filePath)
      } catch (error) {
        logger.error('Failed to get file stream from S3', { error, filePath })
        if (STORAGE_MODE === 's3') {
          throw error
        }
        // Fall through to local storage if hybrid mode
 ...
 * Supports CloudFront CDN, S3, and local storage
 */
export function getFileURL(relativePath: string): string {
  // Use CloudFront CDN if enabled
  ...
  try {
    ...
    if (isS3Key && (STORAGE_MODE === 's3' || STORAGE_MODE === 'hybrid')) {
      try {
        ...
        logger.debug('File deleted from S3', { filePath })
      } catch (error) {
        logger.error('Failed to delete file from S3', { error, filePath })
        if (STORAGE_MODE === 's3') {
          throw error
        }
        // Continue with local deletion if hybrid mode
      }
    }

    // Delete from local storage if not S3-only mode
    if (STORAGE_MODE === 'local' || STORAGE_MODE === 'hybrid') {
      const fullPath = path.isAbsolute(filePath)
        ? filePath
        : path.join(STORAGE_ROOT, filePath)
      ...
      if (!exists) {
        logger.warn('File does not exist locally, skipping deletion', { filePath })
        return
      }

      await fs.unlink(fullPath)
      logger.debug('File deleted locally', { filePath })
    }
  } catch (error) {
