...
export const corsOptions: cors.CorsOptions = {
  ...
  exposedHeaders: ['X-Total-Count', 'X-Page-Count', 'X-Session-Id'],
...
export const helmetConfig = helmet({
  contentSecurityPolicy: isDevelopment ? devContentSecurityPolicy : prodContentSecurityPolicy,
...
export const generalRateLimit = rateLimit({
  ...
  skip: (req) => process.env.NODE_ENV === 'development' && (req.ip === '::1' || req.ip === '127.0.0.1') || process.env.DB_MOCK === 'true',
  ...(redisClient && {
...
export const paymentRateLimit = rateLimit({
  ...
  legacyHeaders: false,
  keyGenerator: rateLimitKeyGenerator,
  skip: (req) => process.env.DB_MOCK === 'true',
  ...(redisClient && {
    store: new RedisStore({
      // @ts-expect-error
      client: redisClient,
