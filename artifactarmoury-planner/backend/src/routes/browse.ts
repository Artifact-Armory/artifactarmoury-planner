  ...
  asyncHandler(async (req, res) => {
    ...
    if (IS_MOCK_DB) {
      ...
      const categoryValue = Array.isArray(category)
        ? (category[0] !== undefined ? String(category[0]) : undefined)
      ...
      const searchValue = Array.isArray(search)
        ? (search[0] !== undefined ? String(search[0]) : '')
    ...
    const result = await db.query(
      `SELECT
        ...
        m.thumbnail_path, m.base_price, m.license,
        m.width, m.height, m.depth,
        m.view_count, m.sale_count,
        m.published_at, m.glb_file_path,
        u.artist_name, u.artist_url, u.creator_verified, u.verification_badge,
       ...
       WHERE ${whereClause}
       GROUP BY m.id, m.license, m.glb_file_path, u.artist_name, u.artist_url, u.creator_verified, u.verification_badge, a.id
       ORDER BY ${orderBy}
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await db.query(
      `SELECT
        ...
        m.thumbnail_path, m.base_price, m.license, m.glb_file_path,
        u.artist_name, u.artist_url, u.creator_verified, u.verification_badge,
        COUNT(DISTINCT r.id) as review_count,
        COALESCE(AVG(r.rating), 0) as average_rating,
        m.sale_count,
