...
import { createMockUser, getMockUserByEmail, getMockUserById, sanitizeMockUser, setMockUserPassword } from '../mock/mockUsers';
import { SESSION_HEADER } from '../middleware/session';

const IS_MOCK_DB = process.env.DB_MOCK === 'true';

const router = Router();

async function mergeAnonymousSessionTables(sessionId: string, userId: string) {
  if (!sessionId || IS_MOCK_DB) return;
  await db.query(
    `
      UPDATE tables
      SET user_id = $1,
  ...
  await db
    .query(`DELETE FROM anonymous_sessions WHERE session_token = $1`, [sessionId])
    .catch(() => undefined);
}

async function bindSessionToUser(req: AuthRequest, res: import('express').Response, userId: string) {
  const sessionInfo = req.session;
  if (sessionInfo?.isAnonymous && sessionInfo.id) {
    await mergeAnonymousSessionTables(sessionInfo.id, userId);
  }

  res.setHeader(SESSION_HEADER, userId);
}
...
router.post('/register', authRateLimit, asyncHandler(async (req: AuthRequest, res) => {
  const { email, password, displayName, artistName } = req.body;
  ...
  if (IS_MOCK_DB) {
    ...
    const display = sanitizeString(displayName);
    const artistLabel = sanitizeString(artistName || displayName);
    ...
    const user = sanitizeMockUser(mockUser);

    const accessToken = generateToken(user);
    const refreshToken = generateRefreshToken(user);

    logger.info('Mock user registered', { userId: user.id, email: user.email });

    await bindSessionToUser(req, res, user.id)
    ...
    res.status(201).json({
      ...
      user: {
        ...
        role: user.role,
        creatorVerified: Boolean(user.creator_verified),
  ...
  // Create user
  const normalizedDisplayName = sanitizeString(displayName);
  const normalizedArtistName = sanitizeString(artistName || displayName);

  const result = await db.query(
    `INSERT INTO users (email, password_hash, display_name, artist_name, role)
     VALUES ($1, $2, $3, $4, 'artist')
     RETURNING id, email, display_name, artist_name, role, created_at, creator_verified, verification_badge`,
    [email.toLowerCase(), passwordHash, normalizedDisplayName, normalizedArtistName]
  );
...
router.post('/register/artist', authRateLimit, asyncHandler(async (req: AuthRequest, res) => {
  const { email, password, displayName, artistName, inviteCode } = req.body;

  // Validate input
  if (!email || !password || !displayName || !artistName) {
    throw new ValidationError('Email, password, display name, and artist name are required');
  }
  ...
  if (IS_MOCK_DB) {
    ...
    logger.info('Mock artist registered', { userId: user.id, email: user.email })
  ...
  try {
    ...
    const userResult = await client.query(
      `INSERT INTO users (email, password_hash, display_name, role, artist_name)
       ...
       RETURNING id, email, display_name, role, artist_name, created_at, creator_verified, verification_badge`,
    ...
    await client.query(
      ...
      [user.id, JSON.stringify({ inviteCode: inviteCode || null }), req.ip, req.get('user-agent')]
    );
...
router.post('/login', authRateLimit, asyncHandler(async (req: AuthRequest, res) => {
  ...
  if (IS_MOCK_DB) {
    ...
    if (!isValidPassword) {
      logger.warn('Failed mock login attempt', { email, ip: req.ip })
      throw new AuthenticationError('Invalid email or password')
    }

    if (mockUser.account_status === 'suspended') {
      throw new AuthenticationError('Your account has been suspended. Please contact support.')
    }

    if (mockUser.account_status === 'banned') {
    ...
    logger.info('Mock user logged in', { userId: user.id, email: user.email })
  ...
  const result = await db.query(
    `SELECT id, email, password_hash, display_name, role, account_status,
            ...
            creator_verified, verification_badge
     FROM users 
     WHERE email = $1`,
    [email.toLowerCase()]
  );
...
router.get('/me', authenticate, asyncHandler(async (req, res) => {
  if (IS_MOCK_DB) {
    const mockUser = getMockUserById((req as any).userId)
    if (!mockUser) {
...
router.post('/password-reset/request', emailRateLimit, asyncHandler(async (req, res) => {
  ...
  if (IS_MOCK_DB) {
    logger.info('Mock password reset request', { email });
...
router.post('/password/change', authenticate, asyncHandler(async (req, res) => {
  ...
  if (IS_MOCK_DB) {
    ...
    const isValid = await bcrypt.compare(currentPassword, mockUser.password_hash);
    if (!isValid) {
      throw new AuthenticationError('Current password is incorrect');
    }
    await setMockUserPassword(userId, newPassword);
    logger.info('Mock password changed', { userId });
...
router.post('/invite/verify', asyncHandler(async (req, res) => {
  ...
  if (IS_MOCK_DB) {
    res.json({ valid: true, message: 'Invite code valid (mock)', remainingUses: Infinity });
    return;
  }
