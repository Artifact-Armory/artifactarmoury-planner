...
// Asset library API (global catalogue, search, tracking)
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    if (IS_MOCK_DB) {
      // In mock mode, just return success without tracking
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!asset) {
      ...
      logger.info('Asset auto-created from model for planning library', {
        modelId,
        assetId: asset.id,
      })
    }
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    res.json({
      ...
      affectedTables: deleteResult.rows.map((row) => row.table_id),
      modelId,
    })
  }),
)
