...
- STLs gain invisible mesh nudges plus metadata headers identifying the artist and watermark token.
- GLB previews include matching extras (`artifactArmouryWatermark`) for provenance.
- Preview images (auto-generated or uploaded thumbnails) are watermarked prior to storage.
- Each upload stores a feature vector and signature in `model_watermarks` for similarity detection.
- Admin investigation endpoint: `GET /api/models/:id/similar`.
- Full workflow and support playbook live in `docs/watermarking.md`.

#### Licensing & Creator Trust
- Models require an explicit `license` (CC variants, personal, commercial) and expose it through every catalog endpoint.
- Artists can earn verification badges (`creator_verified`, `verification_badge`) surfaced in browse, detail, and artist profile views.
- Frontend utilities for rendering license labels live in `frontend/src/utils/licenses.ts`.
...
1. Create a new SQL file under `backend/db/migrations` using the naming convention `###_description.sql`.
2. Keep migrations sequential and idempotent wherever possible.
3. Test migrations locally before opening a pull request.

```bash
touch backend/db/migrations/00X_add_feature.sql
```

### Asset Library System

The builder includes a table-scoped asset library that lets users curate reusable models while planning layouts.

- **Anonymous sessions**: generated automatically (`x-session-id`) with a three-table limit by default.
- **Authenticated users**: tables persist to the account and inherit higher plan limits.
- **Assets**: global catalogue entries uploaded by artists and referenced by tables.
- **Sets**: curated collections grouping multiple assets.
- **Table Library**: the subset of assets attached to an individual table for quick reuse.

See `docs/MIGRATION_GUIDE.md` for schema details and operational guidance.
...
- Set `DB_MOCK=true` to run the API without PostgreSQL.
- Credentials persist only in memory; the server seeds `demo@artifactarmoury.com / demo123` for quick access.
- Registration and login endpoints operate against the in-memory store while mock mode is enabled.
