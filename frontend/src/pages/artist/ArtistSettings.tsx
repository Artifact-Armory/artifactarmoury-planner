  ...
  return (
    <div className="px-4 py-10 max-w-3xl mx-auto">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Artist Profile</h1>
          <p className="text-muted-foreground">Your public brand page — banner, background, avatar, colour, and bio.</p>
        </div>
        {user?.id && (
          <a
      ...
      <div className="mt-6">
        ...
        <div className="relative overflow-hidden rounded-2xl border shadow-xs">
          ...
          {/* hero card */}
          <div className="relative m-3 overflow-hidden rounded-xl bg-card/95 shadow-sm">
            ...
            <div className="flex items-end gap-3 px-4 pb-4">
              <div className="-mt-8 h-16 w-16 flex-none overflow-hidden rounded-full border-4 border-card bg-muted">
                {avatarUrl
                  ? <img src={avatarUrl} alt="" className="h-full w-full object-cover" />
                  : <div className="flex h-full w-full items-center justify-center text-sm font-semibold text-muted-foreground">{(name || 'AA').slice(0, 2).toUpperCase()}</div>}
              </div>
              <div className="min-w-0 flex-1 pt-2">
                <h3 className="truncate text-lg font-semibold text-foreground">{name || 'Your studio name'}</h3>
                {bio && <p className="mt-1 line-clamp-2 whitespace-pre-line text-xs text-muted-foreground">{bio}</p>}
              </div>
          ...
          {featuredThumbs.length > 0 && (
            <div className="relative mx-3 mb-3">
              <p className="mb-1 text-xs font-medium text-muted-foreground">Featured</p>
      ...
      <form className="mt-6 space-y-5" onSubmit={save}>
        ...
        <div>
          ...
          <div className="relative h-32 w-full overflow-hidden rounded-lg bg-linear-to-r from-primary to-purple-500">
          ...
          {uploading === 'banner' && <span className="ml-2 text-xs text-muted-foreground">Uploading…</span>}
        </div>

        {/* Page background */}
        <div>
          <label className="block text-sm font-medium mb-1">Page background</label>
          <p className="text-xs text-muted-foreground mb-2">A full-page backdrop shown behind your brand page (a soft white overlay keeps text readable).</p>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            {uploading === 'background' && <span className="text-xs text-muted-foreground">Uploading…</span>}
            {backgroundUrl && (
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            {uploading === 'avatar' && <span className="text-xs text-muted-foreground">Uploading…</span>}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-sm text-muted-foreground">{accent || 'Default theme'}</span>
            {accent && (
        ...
        <button type="submit" disabled={saving || uploading !== null} className="rounded-md bg-primary px-5 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-60">
      ...
      <section className="mt-12 border-t pt-8">
        <h2 className="text-xl font-semibold text-foreground">Featured models</h2>
        ...
        {featuredIds.length > 0 && (
          <ol className="mt-4 space-y-2">
            {featuredIds.map((mid, i) => {
              ...
              return (
                <li key={mid} className="flex items-center gap-3 rounded-lg border bg-card p-2">
                  ...
                  <span className="min-w-0 flex-1 truncate text-sm font-medium text-foreground">{m?.name ?? 'Model'}</span>
                  <button type="button" className="rounded-sm p-1 hover:bg-muted disabled:opacity-30" disabled={i === 0} onClick={() => moveFeatured(i, -1)}><ArrowUp size={16} /></button>
                  <button type="button" className="rounded-sm p-1 hover:bg-muted disabled:opacity-30" disabled={i === featuredIds.length - 1} onClick={() => moveFeatured(i, 1)}><ArrowDown size={16} /></button>
        ...
        <button
          ...
          className="mt-4 rounded-md bg-primary px-5 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-60"
        >
          {savingFeatured ? 'Saving…' : 'Save featured'}
        </button>

        <div className="mt-8 flex flex-wrap items-center justify-between gap-3">
          <h3 className="text-sm font-medium text-foreground">Your published models</h3>
          {(pickerModels.length > 0 || modelSearch.trim()) && (
            <div className="relative">
              ...
              <input
                ...
                className="w-56 rounded-md border border-border py-1.5 pl-8 pr-3 text-sm shadow-xs focus:border-primary focus:outline-hidden"
              />
            </div>
          )}
        </div>
        {pickerModels.length === 0 && modelsFetching ? (
          ...
          <p className="mt-3 text-sm text-muted-foreground">No models match “{modelSearch}”.</p>
        ) : pickerModels.length === 0 ? (
          <p className="mt-2 text-sm text-muted-foreground">Publish a model first, then you can feature it here.</p>
        ) : (
            ...
            {pickerModels.map((m) => {
              ...
              return (
                <button
                  ...
                  className={`group relative overflow-hidden rounded-lg border text-left transition ${active ? 'border-primary ring-2 ring-primary' : 'border-border hover:border-border'}`}
                >
                  <div className="aspect-square w-full bg-muted">
                    {m.thumbnailUrl && <img src={m.thumbnailUrl} alt="" className="h-full w-full object-cover" />}
                  </div>
                  <span className={`absolute right-1.5 top-1.5 rounded-full p-1 ${active ? 'bg-primary text-primary-foreground' : 'bg-card/80 text-muted-foreground'}`}>
                    <Star size={14} fill={active ? 'currentColor' : 'none'} />
                  </span>
                  <span className="block truncate p-2 text-xs font-medium text-foreground">{m.name}</span>
                </button>
              )
            })}
          </div>
