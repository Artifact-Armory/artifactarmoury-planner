    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl bg-card shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b px-6 py-4">
          <h2 className="text-lg font-semibold text-foreground">Report conversation with {otherName}</h2>
        ...
        <div className="space-y-4 px-6 py-5">
          ...
          <div>
            <label className="block text-sm font-medium text-foreground">Why are you reporting this conversation?</label>
            <div className="mt-2 space-y-2">
              {REASONS.map((r) => (
                <label
                  key={r.value}
                  className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 text-sm ${
                    reason === r.value ? 'border-primary/50 bg-primary/10' : 'border-border hover:border-primary/40'
                  }`}
                >
          ...
          <div>
            <label className="block text-sm font-medium text-foreground">Anything else? (optional)</label>
            <textarea
              value={detail}
