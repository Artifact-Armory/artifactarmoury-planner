...
export interface BasketItem {
  ...
  firstQty?: number // How many at first purchase price (STL: always 1, Print: 1)
  repeatQty?: number // How many at repeat purchase price (STL: always 0, Print: quantity-1)
}
...
export function calculateBasketTotal(
  ...
  const items = basketItems.map(item => {
    ...
    return {
      ...
      fulfillment: item.fulfillment ?? 'print',
      isFirstPurchase,
      unitPrice,
      lineTotal,
      firstQty,
